...
export const VcsKeySection: React.FC = () => {
  const { updateVcsKey, hasVcsKey } = useSettings();
    ...
    >
      <div className="flex items-center justify-between pb-3 border-b border-slate-800">
        <div className="flex items-center space-x-2">
          ...
          <h2 className="text-sm font-extrabold text-white">VCS</h2>
        </div>
      ...
      {/* Hinweis zu benötigten Token-Scopes */}
      <div className="p-3.5 bg-slate-800/60 border border-slate-700/60 rounded-xl flex items-start space-x-3 text-xs text-slate-300">
        ...
        <div className="space-y-1">
          ...
          <p className="text-slate-400 leading-relaxed">
            ...
            Scopes:{" "}
            <code className="px-1.5 py-0.5 bg-slate-950 text-orange-400 rounded text-[11px] font-mono border border-slate-700">
              read_user
            </code>
            ,{" "}
            <code className="px-1.5 py-0.5 bg-slate-950 text-orange-400 rounded text-[11px] font-mono border border-slate-700">
      ...
      <div className="text-xs">
        <label className="block font-bold text-slate-300 mb-1.5 flex items-center space-x-1">
          ...
          <span>VCS API Key (Personal Access Token)</span>
        </label>
      ...
      <div className="flex justify-end pt-2">
        ...
        >
          ...
          <span>{hasVcsKey ? "Overwrite VCS Key" : "Save VCS API Key"}</span>
        </button>
      </div>
    </form>
  );
