...
import { ArrowLeft, Lock, LogOut, ShieldAlert, Sparkles } from "lucide-react";
...
> = ({ children, minPlan }) => {
  ...
  useEffect(() => {
    // Wenn kein Zugriff besteht und nicht mehr geladen wird: 5 Sekunden Delay starten
    ...
    // Falls Zugriff besteht oder geladen wird: Overlay direkt deaktivieren
    setShowRestrictedOverlay(false);
  }, [isAllowed, isLoading]);

  if (isLoading || isAllowed || !showRestrictedOverlay) {
    return <>{children}</>;
  }

  return (
    <div className="fixed inset-0 z-[999999] bg-[#020617] text-white flex flex-col items-center justify-center p-6 text-center select-none font-sans overflow-hidden animate-in fade-in duration-300">
      ...
      <div className="relative z-10 bg-[#0f172a] border border-slate-800 rounded-3xl p-8 max-w-md w-full shadow-2xl flex flex-col items-center animate-in zoom-in-95 duration-200">
        {/* Status Badge */}
        ...
        <button
          type="button"
          onClick={logout}
          className="w-full pt-3 border-t border-slate-800/80 flex items-center justify-center gap-1.5 text-xs text-slate-400 hover:text-red-400 transition-colors cursor-pointer"
        >
