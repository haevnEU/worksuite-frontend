...
> = ({ children, minPlan, upgradeUrl = "/license/plans" }) => {
  ...
  if (!isAllowed) {
    // Token an den Link zur Thymeleaf-Seite anhängen
    const targetUpgradeHref = token
        ? `${upgradeUrl}${upgradeUrl.includes("?") ? "&" : "?"}token=${encodeURIComponent(token)}`
        : upgradeUrl;
